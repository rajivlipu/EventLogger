...
namespace EventLogger
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            Timer99.Interval = new TimeSpan(0, 0, 0, 15, 0);
        ...
        private async void Timer99_Tick(object sender, EventArgs e)
        {
            ...
            if (percentage < 5)
            {
                ...
                await callLLMAsync("Hard disk utilization is " + harddiskper.ToString() + " percentage");
            }
            if (CpuData.Count > 0)
            {
                ...
                {
                    await callLLMAsync("CPU usage greater than 90 percentage for more than 30 second");
                }
            }
            if (MemData.Count > 0)
            {
                ...
                if (span.TotalSeconds > 60)
                {
                    await callLLMAsync("Memory usage greater than 90 percentage for more than 30 second");
                }
            }
            if (DiskData.Count > 0)
            {
                ...
                if (span.TotalSeconds > 60)
                {
                    await callLLMAsync("Disk usage greater than 90 percentage for more than 30 second");
                }
            }
...
";
        ...
        static async void GetSNT()
        {
            ...
            try
            {
                ...
                await new Form1().callLLMAsync(data1);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public async Task callLLMAsync(string query)
        {
            ...
            if (action == "create ticket")
            {
                ...
                await CreateTicket(shortDesc, comment);
            }
            if (action == "update ticket")
            {
                string sysId = data["response"]["sysid"].Value<string>();
